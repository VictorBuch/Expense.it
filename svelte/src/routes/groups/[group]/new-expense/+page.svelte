<script>
	...
	let users = [
		{ name: 'john', number: '+45 31784920' },
		{ name: 'legend', number: '+45 31774280' },
		{ name: 'legend', number: '+45 31774280' },
		{ name: 'legend', number: '+45 31774280' },
		{ name: 'legend', number: '+45 31774280' },
		{ name: 'Jimmy', number: '+45 89372594' }
	];
</script>
...
<main>
	...
	<div class="my-4 container flex flex-col items-center space-y-2 overflow-x-hidden">
		...
		<div class="flex items-center space-x-6 w-full overflow-x-hidden user-payment">
			{#each users as user}
				<div class="avatar">
		...
		<p class="text-base-content text-lg">I pay</p>
	</div>
	<div
	...
	>
		...
		<div onclick="changeCurrecy" class="flex items-center cursor-pointer space-x-4">
			<p class="leading-3 font-xs">kr.</p>
			<i class="fa-solid fa-circle-chevron-down text-primary" />
		</div>
		<i onclick="remove" class="fa-solid fa-delete-left absolute right-4 cursor-pointer" />
	</div>
	...
	<div class="mt-8">
		{#each users as user}
			<div class="py-1 px-2 cursor-pointer flex items-center bg-base-100">
				...
				<div class="ml-2">
					<p class="text-lg leading-5 font-bold text-base-content">{user.name}</p>
					<p class="text-xs leading-5 text-base-content">{user.number}</p>
				</div>
